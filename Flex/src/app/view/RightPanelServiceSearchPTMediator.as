package app.view
{
	...
	public class RightPanelServiceSearchPTMediator extends Mediator implements IMediator
	{
		public static const NAME:String = "RightPanelServiceSearchPTMediator";
				
		//private var listOriginRoad:ArrayCollection = null;
		
		public function RightPanelServiceSearchPTMediator(viewComponent:Object=null)
		{
			super(NAME, viewComponent);
			
			rightPanelServiceSearch.addEventListener(RightPanelServiceSearchPT.COMBOTEXTCHANGE,onComboTextChange);
			
			//rightPanelServiceSearch.addEventListener(RightPanelServiceSearch.ROAD2TEXTCHANGE,onComboTextChange);			
			rightPanelServiceSearch.addEventListener(RightPanelServiceSearchPT.CROSSROADCHANGE,onCrossRoadChange);			
			...
			rightPanelServiceSearch.addEventListener(RightPanelServiceSearchPT.ADDRESSTEXTCHANGE,onAddressTextChange);
			rightPanelServiceSearch.addEventListener(RightPanelServiceSearchPT.RADIOCUSTOMCHANGE,onRadioCustomChange);
						
			rightPanelServiceSearch.addEventListener(RightPanelServiceSearchPT.MAPLOCATE,onMapOperator);
			rightPanelServiceSearch.addEventListener(RightPanelServiceSearchPT.MAPSEARCH,onMapOperator);
			
			rightPanelServiceSearch.addEventListener(RightPanelServiceSearchPT.GRAPHICCHANGE,onBarGraphicChange);
									
			rightPanelServiceSearch.addEventListener(RightPanelServiceSearchPT.SEARCH,onSearchPolice);
		}
		
		protected function get rightPanelServiceSearch():RightPanelServiceSearchPT
		{
			return viewComponent as RightPanelServiceSearchPT;
		}
		...
		{						
			...
			function queryResultHandle(featureSet:FeatureSet):void
			{
				...
				{			
					...
					if(type == RightPanelServiceSearchPT.MAPLOCATE)
					{
						sendNotification(AppNotification.NOTIFY_SEARCH_LOCATEROAD,road);
					}
					else
		...
		{			
			...
			for each(var gps:GPSNewVO in gpsRealTimeInfoProxy.listGPS)
			{
				if(
					...
					&&
					(
						(rightPanelServiceSearch.listKindItem == "所有警种")
						|| (gps.policeKind == rightPanelServiceSearch.listKindItem)
					)
					&& 
			...
			switch(notification.getName())
			{
				...
				case AppNotification.NOTIFY_MENUBAR:
					if(notification.getType() == MainMenu.SERVICESEARCH)
					{
						...
						rightPanelServiceSearch.listKind.removeAll();
						rightPanelServiceSearch.listKind.addItem("所有警种");
						rightPanelServiceSearch.listKindItem = "所有警种";
						
						onSearchPolice();
						
						for each(var gps:GPSNewVO in rightPanelServiceSearch.listPolice)
						{
							if(!rightPanelServiceSearch.listKind.contains(gps.policeKind))
								rightPanelServiceSearch.listKind.addItem(gps.policeKind);							
						}
						
						/*if(rightPanelServiceSearch.listKindItem != "所有警种"
							&& rightPanelServiceSearch.listKind.contains(rightPanelServiceSearch.listKindItem))
						{				
							for each(gps in listPoliceTemp)
							{
								if(gps.policeKind == rightPanelServiceSearch.listKindItem)
								{				
								}
							}
						}
						...
						{
							rightPanelServiceSearch.listPolice.addAll(listPoliceTemp);
						}*/
					}
					break;
